...
The analysis was performed separately for distinct feature subsets $\mathcal{F}_c$, each clustered to reflect the behaviour by specific SBDT components. Based on the feature engineering described in \autoref{sec:feature-engineering}, subsets were defined for the \texttt{time\_model} (including duration, sequence, cyclical time features), \texttt{resource\_model} (categorical resource, part, process IDs), \texttt{process\_model} (process ID, duration, sequence), \texttt{kpi\_based} (calculated KPIs like throughput, cycle time), and an \texttt{all\_features} set containing all available engineered features. PRÜFEN UND ANPASSEN
